 ...
 */
final class Version20190329144931 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('DROP INDEX IDX_AD52EF56A53A8AA');
        $this->addSql('DROP INDEX IDX_AD52EF5619EB6921');
        $this->addSql('CREATE TEMPORARY TABLE __temp__historic AS SELECT id, client_id, provider_id, detail, date, total FROM historic');
        $this->addSql('DROP TABLE historic');
        $this->addSql('CREATE TABLE historic (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, client_id INTEGER DEFAULT NULL, provider_id INTEGER DEFAULT NULL, detail VARCHAR(255) NOT NULL COLLATE BINARY, date DATETIME NOT NULL, total NUMERIC(5, 2) NOT NULL, CONSTRAINT FK_AD52EF5619EB6921 FOREIGN KEY (client_id) REFERENCES client (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_AD52EF56A53A8AA FOREIGN KEY (provider_id) REFERENCES provider (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO historic (id, client_id, provider_id, detail, date, total) SELECT id, client_id, provider_id, detail, date, total FROM __temp__historic');
        $this->addSql('DROP TABLE __temp__historic');
        $this->addSql('CREATE INDEX IDX_AD52EF56A53A8AA ON historic (provider_id)');
        $this->addSql('CREATE INDEX IDX_AD52EF5619EB6921 ON historic (client_id)');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__historic AS SELECT id, client_id, provider_id, detail, date, total FROM historic');
        $this->addSql('DROP TABLE historic');
        $this->addSql('CREATE TABLE historic (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, client_id INTEGER DEFAULT NULL, provider_id INTEGER DEFAULT NULL, detail VARCHAR(255) NOT NULL, date DATETIME NOT NULL, total NUMERIC(5, 2) NOT NULL)');
        $this->addSql('INSERT INTO historic (id, client_id, provider_id, detail, date, total) SELECT id, client_id, provider_id, detail, date, total FROM __temp__historic');
        $this->addSql('DROP TABLE __temp__historic');
        $this->addSql('CREATE INDEX IDX_AD52EF5619EB6921 ON historic (client_id)');
        $this->addSql('CREATE INDEX IDX_AD52EF56A53A8AA ON historic (provider_id)');
